  ...
  override def loadTable(ident: Identifier): Table = {
    super.loadTable(ident) match {
      case V1Table(catalogTable0) if sparkAdapter.isHoodieTable(catalogTable0) =>
        ...
        // hct construction is cheap; HoodieCatalogTable.metaClient is itself a lazy val
        // so the filesystem probe only fires when the gate actually consults it. Skipped
        // entirely when schemaEvolutionEnabled forces the V1 fallback.
        ...
        val dsv2Supported = !schemaEvolutionEnabled && {
          ...
          v2ReadEnabled && HoodieV2ReadSupport.isSupportedByDSv2(hct.metaClient, resolvedOpts)
        }
