  ...
  private val commitsTimelineToReturn = {
    if (hollowCommitHandling == USE_TRANSITION_TIME) {
      commitTimeline.findInstantsInRangeByCompletionTime(
        optParams(DataSourceReadOptions.START_COMMIT.key),
        optParams.getOrElse(DataSourceReadOptions.END_COMMIT.key(), lastInstant.getCompletionTime))
    } else {
      commitTimeline.findInstantsInRange(
        optParams(DataSourceReadOptions.START_COMMIT.key),
        optParams.getOrElse(DataSourceReadOptions.END_COMMIT.key(), lastInstant.requestedTime))
    }
  }
